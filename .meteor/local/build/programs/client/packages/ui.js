...
UI.insert = function (renderedTemplate, parentElement, nextNode) {                                            // 172
  // parentElement must be a DOM node. in particular, can't be the                                            // 173
  // result of a call to `$`. Can't check if `parentElement instanceof                                        // 174
  // Node` since 'Node' is undefined in IE8.                                                                  // 175
  if (! parentElement || typeof parentElement.nodeType !== 'number')                                          // 176
    throw new Error("'parentElement' must be a DOM node");                                                    // 177
  if (nextNode && typeof nextNode.nodeType !== 'number') // 'nextNode' is optional                            // 178
    throw new Error("'nextNode' must be a DOM node");                                                         // 179
                                                                                                              // 180
  if (! renderedTemplate.dom)                                                                                 // 181
    throw new Error("Expected template rendered with UI.render");                                             // 182
                                                                                                              // 183
  UI.DomRange.insert(renderedTemplate.dom, parentElement, nextNode);                                          // 184
};                                                                                                            // 185
                                                                                                              // 186
// Insert a DOM node or DomRange into a DOM element or DomRange.                                              // 187
//                                                                                                            // 188
// One of three things happens depending on what needs to be inserted into what:                              // 189
// - `range.add` (anything into DomRange)                                                                     // 190
// - `UI.DomRange.insert` (DomRange into element)                                                             // 191
// - `elem.insertBefore` (node into element)                                                                  // 192
//                                                                                                            // 193
// The optional `before` argument is an existing node or id to insert before in                               // 194
// the parent element or DomRange.                                                                            // 195
var insert = function (nodeOrRange, parent, before) {                                                         // 196
  if (! parent)                                                                                               // 197
    throw new Error("Materialization parent required");                                                       // 198
                                                                                                              // 199
  if (parent instanceof UI.DomRange) {                                                                        // 200
    parent.add(nodeOrRange, before);                                                                          // 201
  } else if (nodeOrRange instanceof UI.DomRange) {                                                            // 202
    // parent is an element; inserting a range                                                                // 203
    UI.DomRange.insert(nodeOrRange, parent, before);                                                          // 204
  } else {                                                                                                    // 205
    // parent is an element; inserting an element                                                             // 206
    parent.insertBefore(nodeOrRange, before || null); // `null` for IE                                        // 207
  }                                                                                                           // 208
};                                                                                                            // 209
                                                                                                              // 210
// options include:                                                                                           // 211
//   - _nestInCurrentComputation: defaults to false. If true, then                                            // 212
//     `render`'s autoruns will be nested inside the current                                                  // 213
//     computation, so if the current computation is invalidated, then                                        // 214
//     the autoruns set up inside `render` will be stopped. If false,                                         // 215
//     the autoruns will be set up in a fresh Deps context, so                                                // 216
//     invalidating the current computation will have no effect on them.                                      // 217
UI.render = function (kind, parentComponent, options) {                                                       // 218
  options = options || {};                                                                                    // 219
                                                                                                              // 220
  if (kind.isInited)                                                                                          // 221
    throw new Error("Can't render component instance, only component kind");                                  // 222
                                                                                                              // 223
  var inst, content, range;                                                                                   // 224
                                                                                                              // 225
  Deps.nonreactive(function () {                                                                              // 226
                                                                                                              // 227
    inst = kind.instantiate(parentComponent);                                                                 // 228
                                                                                                              // 229
    content = (inst.render && inst.render());                                                                 // 230
                                                                                                              // 231
    range = new UI.DomRange;                                                                                  // 232
    inst.dom = range;                                                                                         // 233
    range.component = inst;                                                                                   // 234
                                                                                                              // 235
    if (! options._nestInCurrentComputation) {                                                                // 236
      materialize(content, range, null, inst);                                                                // 237
    }                                                                                                         // 238
                                                                                                              // 239
  });                                                                                                         // 240
                                                                                                              // 241
  if (options._nestInCurrentComputation) {                                                                    // 242
    materialize(content, range, null, inst);                                                                  // 243
  }                                                                                                           // 244
                                                                                                              // 245
  range.removed = function () {                                                                               // 246
    inst.isDestroyed = true;                                                                                  // 247
    if (inst.destroyed) {                                                                                     // 248
      Deps.nonreactive(function () {                                                                          // 249
        updateTemplateInstance(inst);                                                                         // 250
        inst.destroyed.call(inst.templateInstance);                                                           // 251
      });                                                                                                     // 252
    }                                                                                                         // 253
  };                                                                                                          // 254
                                                                                                              // 255
  return inst;                                                                                                // 256
                                                                                                              ...
                                                                                                              // 258
// options are the same as for UI.render.                                                                     // 259
UI.renderWithData = function (kind, data, parentComponent, options) {                                         // 260
  if (! UI.isComponent(kind))                                                                                 // 261
    throw new Error("Component required here");                                                               // 262
  if (kind.isInited)                                                                                          // 263
    throw new Error("Can't render component instance, only component kind");                                  // 264
  if (typeof data === 'function')                                                                             // 265
    throw new Error("Data argument can't be a function");                                                     // 266
                                                                                                              // 267
  return UI.render(kind.extend({data: function () { return data; }}),                                         // 268
                   parentComponent, options);                                                                 // 269
};                                                                                                            // 270
                                                                                                              // 271
var contentEquals = function (a, b) {                                                                         // 272
  if (a instanceof HTML.Raw) {                                                                                // 273
    return (b instanceof HTML.Raw) && (a.value === b.value);                                                  // 274
  } else if (a == null) {                                                                                     // 275
    return (b == null);                                                                                       // 276
  } else {                                                                                                    // 277
    return (a === b) &&                                                                                       // 278
      ((typeof a === 'number') || (typeof a === 'boolean') ||                                                 // 279
       (typeof a === 'string'));                                                                              // 280
  }                                                                                                           // 281
};                                                                                                            // 282
                                                                                                              // 283
UI.InTemplateScope = function (tmplInstance, content) {                                                       // 284
  if (! (this instanceof UI.InTemplateScope))                                                                 // 285
    // called without `new`                                                                                   // 286
    return new UI.InTemplateScope(tmplInstance, content);                                                     // 287
                                                                                                              // 288
  var parentPtr = tmplInstance.parent;                                                                        // 289
  if (parentPtr.__isTemplateWith)                                                                             // 290
    parentPtr = parentPtr.parent;                                                                             // 291
                                                                                                              // 292
  this.parentPtr = parentPtr;                                                                                 // 293
  this.content = content;                                                                                     // 294
};                                                                                                            // 295
                                                                                                              // 296
UI.InTemplateScope.prototype.toHTML = function (parentComponent) {                                            // 297
  return HTML.toHTML(this.content, this.parentPtr);                                                           // 298
};                                                                                                            // 299
                                                                                                              // 300
UI.InTemplateScope.prototype.toText = function (textMode, parentComponent) {                                  // 301
  return HTML.toText(this.content, textMode, this.parentPtr);                                                 // 302
};                                                                                                            // 303
                                                                                                              // 304
// Convert the pseudoDOM `node` into reactive DOM nodes and insert them                                       // 305
// into the element or DomRange `parent`, before the node or id `before`.                                     // 306
var materialize = function (node, parent, before, parentComponent) {                                          // 307
  // XXX should do more error-checking for the case where user is supplying the tags.                         // 308
  // For example, check that CharRef has `html` and `str` properties and no content.                          // 309
  // Check that Comment has a single string child and no attributes.  Etc.                                    // 310
                                                                                                              // 311
  if (node == null) {                                                                                         // 312
    // null or undefined.                                                                                     // 313
    // do nothinge.                                                                                           // 314
  } else if ((typeof node === 'string') || (typeof node === 'boolean') || (typeof node === 'number')) {       // 315
    node = String(node);                                                                                      // 316
    insert(document.createTextNode(node), parent, before);                                                    // 317
  } else if (node instanceof Array) {                                                                         // 318
    for (var i = 0; i < node.length; i++)                                                                     // 319
      materialize(node[i], parent, before, parentComponent);                                                  // 320
  } else if (typeof node === 'function') {                                                                    // 321
                                                                                                              // 322
    var range = new UI.DomRange;                                                                              // 323
    var lastContent = null;                                                                                   // 324
    var rangeUpdater = Deps.autorun(function (c) {                                                            // 325
      var content = node();                                                                                   // 326
      // normalize content a little, for easier comparison                                                    // 327
      if (HTML.isNully(content))                                                                              // 328
        content = null;                                                                                       // 329
      else if ((content instanceof Array) && content.length === 1)                                            // 330
        content = content[0];                                                                                 // 331
                                                                                                              // 332
      // update if content is different from last time                                                        // 333
      if (! contentEquals(content, lastContent)) {                                                            // 334
        lastContent = content;                                                                                // 335
                                                                                                              // 336
        if (! c.firstRun)                                                                                     // 337
          range.removeAll();                                                                                  // 338
                                                                                                              // 339
        materialize(content, range, null, parentComponent);                                                   // 340
      }                                                                                                       // 341
    });                                                                                                       // 342
    range.removed = function () {                                                                             // 343
      rangeUpdater.stop();                                                                                    // 344
      if (node.stop)                                                                                          // 345
        node.stop();                                                                                          // 346
    };                                                                                                        // 347
    // XXXX HACK                                                                                              // 348
    if (Deps.active && node.stop) {                                                                           // 349
      Deps.onInvalidate(function () {                                                                         // 350
        node.stop();                                                                                          // 351
      });                                                                                                     // 352
    }                                                                                                         // 353
    insert(range, parent, before);                                                                            // 354
  } else if (node instanceof HTML.Tag) {                                                                      // 355
    var tagName = node.tagName;                                                                               // 356
    var elem;                                                                                                 // 357
    if (HTML.isKnownSVGElement(tagName) && document.createElementNS) {                                        // 358
      elem = document.createElementNS('http://www.w3.org/2000/svg', tagName);                                 // 359
    } else {                                                                                                  // 360
      elem = document.createElement(node.tagName);                                                            // 361
    }                                                                                                         // 362
                                                                                                              // 363
    var rawAttrs = node.attrs;                                                                                // 364
    var children = node.children;                                                                             // 365
    if (node.tagName === 'textarea') {                                                                        // 366
      rawAttrs = (rawAttrs || {});                                                                            // 367
      rawAttrs.value = children;                                                                              // 368
      children = [];                                                                                          // 369
    };                                                                                                        // 370
                                                                                                              // 371
    if (rawAttrs) {                                                                                           // 372
      var attrComp = Deps.autorun(function (c) {                                                              // 373
        var attrUpdater = c.updater;                                                                          // 374
        if (! attrUpdater) {                                                                                  // 375
          attrUpdater = c.updater = new ElementAttributesUpdater(elem);                                       // 376
        }                                                                                                     // 377
                                                                                                              // 378
        try {                                                                                                 // 379
          var attrs = HTML.evaluateAttributes(rawAttrs, parentComponent);                                     // 380
          var stringAttrs = {};                                                                               // 381
          if (attrs) {                                                                                        // 382
            for (var k in attrs) {                                                                            // 383
              stringAttrs[k] = HTML.toText(attrs[k], HTML.TEXTMODE.STRING,                                    // 384
                                           parentComponent);                                                  // 385
            }                                                                                                 // 386
            attrUpdater.update(stringAttrs);                                                                  // 387
          }                                                                                                   // 388
        } catch (e) {                                                                                         // 389
          reportUIException(e);                                                                               // 390
        }                                                                                                     // 391
      });                                                                                                     // 392
      UI.DomBackend.onElementTeardown(elem, function () {                                                     // 393
        attrComp.stop();                                                                                      // 394
      });                                                                                                     // 395
    }                                                                                                         // 396
    materialize(children, elem, null, parentComponent);                                                       // 397
                                                                                                              // 398
    insert(elem, parent, before);                                                                             // 399
  } else if (typeof node.instantiate === 'function') {                                                        // 400
    // component                                                                                              // 401
    var instance = UI.render(node, parentComponent, {                                                         // 402
      _nestInCurrentComputation: true                                                                         // 403
    });                                                                                                       // 404
                                                                                                              // 405
    // Call internal callback, which may take advantage of the current                                        // 406
    // Deps computation.                                                                                      // 407
    if (instance.materialized)                                                                                // 408
      instance.materialized(instance.dom);                                                                    // 409
                                                                                                              // 410
    insert(instance.dom, parent, before);                                                                     // 411
  } else if (node instanceof HTML.CharRef) {                                                                  // 412
    insert(document.createTextNode(node.str), parent, before);                                                // 413
  } else if (node instanceof HTML.Comment) {                                                                  // 414
    insert(document.createComment(node.sanitizedValue), parent, before);                                      // 415
  } else if (node instanceof HTML.Raw) {                                                                      // 416
    // Get an array of DOM nodes by using the browser's HTML parser                                           // 417
    // (like innerHTML).                                                                                      // 418
    var htmlNodes = UI.DomBackend.parseHTML(node.value);                                                      // 419
    for (var i = 0; i < htmlNodes.length; i++)                                                                // 420
      insert(htmlNodes[i], parent, before);                                                                   // 421
  } else if (Package['html-tools'] && (node instanceof Package['html-tools'].HTMLTools.Special)) {            // 422
    throw new Error("Can't materialize Special tag, it's just an intermediate rep");                          // 423
  } else if (node instanceof UI.InTemplateScope) {                                                            // 424
    materialize(node.content, parent, before, node.parentPtr);                                                // 425
  } else {                                                                                                    // 426
    // can't get here                                                                                         // 427
    throw new Error("Unexpected node in htmljs: " + node);                                                    // 428
  }                                                                                                           // 429
};                                                                                                            // 430
                                                                                                              // 431
                                                                                                              // 432
                                                                                                              // 433
// XXX figure out the right names, and namespace, for these.                                                  // 434
// for example, maybe some of them go in the HTML package.                                                    // 435
UI.materialize = materialize;                                                                                 // 436
                                                                                                              // 437
UI.body = UI.Component.extend({                                                                               // 438
  kind: 'body',                                                                                               // 439
  contentParts: [],                                                                                           // 440
  render: function () {                                                                                       // 441
    return this.contentParts;                                                                                 // 442
  },                                                                                                          // 443
  // XXX revisit how body works.                                                                              // 444
  INSTANTIATED: false,                                                                                        // 445
  __helperHost: true                                                                                          // 446
});                                                                                                           // 447
                                                                                                              // 448
UI.block = function (renderFunc) {                                                                            // 449
  return UI.Component.extend({ render: renderFunc });                                                         // 450
};                                                                                                            // 451
                                                                                                              // 452
UI.toHTML = function (content, parentComponent) {                                                             // 453
  return HTML.toHTML(content, parentComponent);                                                               // 454
};                                                                                                            // 455
                                                                                                              // 456
UI.toRawText = function (content, parentComponent) {                                                          // 457
  return HTML.toText(content, HTML.TEXTMODE.STRING, parentComponent);                                         // 458
};                                                                                                            // 459
                                                                                                              // 460
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
UI.EachImpl = Component.extend({                                                                              // 1
  ...
  materialized: function () {                                                                                 // 41
    ...
    this.observeHandle = ObserveSequence.observe(function () {                                                // 70
      ...
      changedAt: function (id, newItem, oldItem, atIndex) {                                                   // 107
...
//# sourceMappingURL=de1b73e223c2aa14155dd1bb8cb1a6d53833f29d.map
