...
var builtInBlockHelpers = {                                                                             // 159
  ...
  'each': 'UI.Each'                                                                                     // 163
                                                                                                        ...
                                                                                                        // 165
// Some `UI.*` paths are special in that they generate code that                                        // 166
// doesn't folow the normal lookup rules for dotted symbols. The                                        // 167
// following names must be prefixed with `UI.` when you use them in a                                   // 168
// template.                                                                                            // 169
var builtInUIPaths = {                                                                                  // 170
  // `template` is a local variable defined in the generated render                                     // 171
  // function for the template in which `UI.contentBlock` (or                                           // 172
  // `UI.elseBlock`) is invoked. `template` is a reference to the                                       // 173
  // template itself.                                                                                   // 174
  'contentBlock': 'template.__content',                                                                 // 175
  'elseBlock': 'template.__elseContent',                                                                // 176
                                                                                                        // 177
  // `Template` is the global template namespace. If you define a                                       // 178
  // template named `foo` in Spacebars, it gets defined as                                              // 179
  // `Template.foo` in JavaScript.                                                                      // 180
  'dynamic': 'Template.__dynamic'                                                                       // 181
};                                                                                                      // 182
                                                                                                        // 183
// A "reserved name" can't be used as a <template> name.  This                                          // 184
// function is used by the template file scanner.                                                       // 185
Spacebars.isReservedName = function (name) {                                                            // 186
  return builtInBlockHelpers.hasOwnProperty(name);                                                      // 187
};                                                                                                      // 188
                                                                                                        // 189
var codeGenTemplateTag = function (tag) {                                                               // 190
  if (tag.position === HTMLTools.TEMPLATE_TAG_POSITION.IN_START_TAG) {                                  // 191
    // only `tag.type === 'DOUBLE'` allowed (by earlier validation)                                     // 192
    return HTML.EmitCode('function () { return ' +                                                      // 193
                         codeGenMustache(tag.path, tag.args, 'attrMustache')                            // 194
                         + '; }');                                                                      // 195
  } else {                                                                                              // 196
    if (tag.type === 'DOUBLE') {                                                                        // 197
      return HTML.EmitCode('function () { return ' +                                                    // 198
                           codeGenMustache(tag.path, tag.args) + '; }');                                // 199
    } else if (tag.type === 'TRIPLE') {                                                                 // 200
      return HTML.EmitCode('function () { return Spacebars.makeRaw(' +                                  // 201
                           codeGenMustache(tag.path, tag.args) + '); }');                               // 202
    } else if (tag.type === 'INCLUSION' || tag.type === 'BLOCKOPEN') {                                  // 203
      var path = tag.path;                                                                              // 204
                                                                                                        // 205
      if (tag.type === 'BLOCKOPEN' &&                                                                   // 206
          builtInBlockHelpers.hasOwnProperty(path[0])) {                                                // 207
        // if, unless, with, each.                                                                      // 208
        //                                                                                              // 209
        // If someone tries to do `{{> if}}`, we don't                                                  // 210
        // get here, but an error is thrown when we try to codegen the path.                            // 211
                                                                                                        // 212
        // Note: If we caught these errors earlier, while scanning, we'd be able to                     // 213
        // provide nice line numbers.                                                                   // 214
        if (path.length > 1)                                                                            // 215
          throw new Error("Unexpected dotted path beginning with " + path[0]);                          // 216
        if (! tag.args.length)                                                                          // 217
          throw new Error("#" + path[0] + " requires an argument");                                     // 218
                                                                                                        // 219
        var codeParts = codeGenInclusionParts(tag);                                                     // 220
        var dataFunc = codeParts.dataFunc; // must exist (tag.args.length > 0)                          // 221
        var contentBlock = codeParts.content; // must exist                                             // 222
        var elseContentBlock = codeParts.elseContent; // may not exist                                  // 223
                                                                                                        // 224
        var callArgs = [dataFunc, contentBlock];                                                        // 225
        if (elseContentBlock)                                                                           // 226
          callArgs.push(elseContentBlock);                                                              // 227
                                                                                                        // 228
        return HTML.EmitCode(                                                                           // 229
          builtInBlockHelpers[path[0]] + '(' + callArgs.join(', ') + ')');                              // 230
                                                                                                        // 231
      } else {                                                                                          // 232
        var compCode = codeGenPath(path, {lookupTemplate: true});                                       // 233
                                                                                                        // 234
        if (path.length !== 1) {                                                                        // 235
          // path code may be reactive; wrap it                                                         // 236
          compCode = 'function () { return ' + compCode + '; }';                                        // 237
        }                                                                                               // 238
                                                                                                        // 239
        var codeParts = codeGenInclusionParts(tag);                                                     // 240
        var dataFunc = codeParts.dataFunc;                                                              // 241
        var content = codeParts.content;                                                                // 242
        var elseContent = codeParts.elseContent;                                                        // 243
                                                                                                        // 244
        var includeArgs = [compCode];                                                                   // 245
        if (content) {                                                                                  // 246
          includeArgs.push(content);                                                                    // 247
          if (elseContent)                                                                              // 248
            includeArgs.push(elseContent);                                                              // 249
        }                                                                                               // 250
                                                                                                        // 251
        var includeCode =                                                                               // 252
              'Spacebars.include(' + includeArgs.join(', ') + ')';                                      // 253
                                                                                                        // 254
        if (dataFunc) {                                                                                 // 255
          includeCode =                                                                                 // 256
            'Spacebars.TemplateWith(' + dataFunc + ', UI.block(' +                                      // 257
            Spacebars.codeGen(HTML.EmitCode(includeCode)) + '))';                                       // 258
        }                                                                                               // 259
                                                                                                        // 260
        if (path[0] === 'UI' &&                                                                         // 261
            (path[1] === 'contentBlock' || path[1] === 'elseBlock')) {                                  // 262
          includeCode = 'UI.InTemplateScope(template, ' + includeCode + ')';                            // 263
        }                                                                                               // 264
                                                                                                        // 265
        return HTML.EmitCode(includeCode);                                                              // 266
      }                                                                                                 // 267
    } else {                                                                                            // 268
      // Can't get here; TemplateTag validation should catch any                                        // 269
      // inappropriate tag types that might come out of the parser.                                     // 270
      throw new Error("Unexpected template tag type: " + tag.type);                                     // 271
    }                                                                                                   // 272
  }                                                                                                     // 273
};                                                                                                      // 274
                                                                                                        // 275
var makeObjectLiteral = function (obj) {                                                                // 276
  var parts = [];                                                                                       // 277
  for (var k in obj)                                                                                    // 278
    parts.push(toObjectLiteralKey(k) + ': ' + obj[k]);                                                  // 279
  return '{' + parts.join(', ') + '}';                                                                  // 280
};                                                                                                      // 281
                                                                                                        // 282
// `path` is an array of at least one string.                                                           // 283
//                                                                                                      // 284
// If `path.length > 1`, the generated code may be reactive                                             // 285
// (i.e. it may invalidate the current computation).                                                    // 286
//                                                                                                      // 287
// No code is generated to call the result if it's a function.                                          // 288
//                                                                                                      // 289
// Options:                                                                                             // 290
//                                                                                                      // 291
// - lookupTemplate {Boolean} If true, generated code also looks in                                     // 292
//   the list of templates. (After helpers, before data context).                                       // 293
//   Used when generating code for `{{> foo}}` or `{{#foo}}`. Only                                      // 294
//   used for non-dotted paths.                                                                         // 295
var codeGenPath = function (path, opts) {                                                               // 296
  if (builtInBlockHelpers.hasOwnProperty(path[0]))                                                      // 297
    throw new Error("Can't use the built-in '" + path[0] + "' here");                                   // 298
  // Let `{{#if UI.contentBlock}}` check whether this template was invoked via                          // 299
  // inclusion or as a block helper, in addition to supporting                                          // 300
  // `{{> UI.contentBlock}}`.                                                                           // 301
  if (path.length >= 2 &&                                                                               // 302
      path[0] === 'UI' && builtInUIPaths.hasOwnProperty(path[1])) {                                     // 303
    if (path.length > 2)                                                                                // 304
      throw new Error("Unexpected dotted path beginning with " +                                        // 305
                      path[0] + '.' + path[1]);                                                         // 306
    return builtInUIPaths[path[1]];                                                                     // 307
  }                                                                                                     // 308
                                                                                                        // 309
  var args = [toJSLiteral(path[0])];                                                                    // 310
  var lookupMethod = 'lookup';                                                                          // 311
  if (opts && opts.lookupTemplate && path.length === 1)                                                 // 312
    lookupMethod = 'lookupTemplate';                                                                    // 313
  var code = 'self.' + lookupMethod + '(' + args.join(', ') + ')';                                      // 314
                                                                                                        // 315
  if (path.length > 1) {                                                                                // 316
    code = 'Spacebars.dot(' + code + ', ' +                                                             // 317
      _.map(path.slice(1), toJSLiteral).join(', ') + ')';                                               // 318
  }                                                                                                     // 319
                                                                                                        // 320
  return code;                                                                                          // 321
};                                                                                                      // 322
                                                                                                        // 323
// Generates code for an `[argType, argValue]` argument spec,                                           // 324
// ignoring the third element (keyword argument name) if present.                                       // 325
//                                                                                                      // 326
// The resulting code may be reactive (in the case of a PATH of                                         // 327
// more than one element) and is not wrapped in a closure.                                              // 328
var codeGenArgValue = function (arg) {                                                                  // 329
  var argType = arg[0];                                                                                 // 330
  var argValue = arg[1];                                                                                // 331
                                                                                                        // 332
  var argCode;                                                                                          // 333
  switch (argType) {                                                                                    // 334
  case 'STRING':                                                                                        // 335
  ...
  case 'BOOLEAN':                                                                                       // 337
  case 'NULL':                                                                                          // 338
    argCode = toJSLiteral(argValue);                                                                    // 339
    break;                                                                                              // 340
  case 'PATH':                                                                                          // 341
    argCode = codeGenPath(argValue);                                                                    // 342
    break;                                                                                              // 343
  default:                                                                                              // 344
    // can't get here                                                                                   // 345
    throw new Error("Unexpected arg type: " + argType);                                                 // 346
  }                                                                                                     // 347
                                                                                                        // 348
  return argCode;                                                                                       // 349
};                                                                                                      // 350
                                                                                                        // 351
// Generates a call to `Spacebars.fooMustache` on evaluated arguments.                                  // 352
// The resulting code has no function literals and must be wrapped in                                   // 353
// one for fine-grained reactivity.                                                                     // 354
var codeGenMustache = function (path, args, mustacheType) {                                             // 355
  var nameCode = codeGenPath(path);                                                                     // 356
  var argCode = codeGenMustacheArgs(args);                                                              // 357
  var mustache = (mustacheType || 'mustache');                                                          // 358
                                                                                                        // 359
  return 'Spacebars.' + mustache + '(' + nameCode +                                                     // 360
    (argCode ? ', ' + argCode.join(', ') : '') + ')';                                                   // 361
};                                                                                                      // 362
                                                                                                        // 363
// returns: array of source strings, or null if no                                                      // 364
// args at all.                                                                                         // 365
var codeGenMustacheArgs = function (tagArgs) {                                                          // 366
  var kwArgs = null; // source -> source                                                                // 367
  var args = null; // [source]                                                                          // 368
                                                                                                        // 369
  // tagArgs may be null                                                                                // 370
  _.each(tagArgs, function (arg) {                                                                      // 371
    var argCode = codeGenArgValue(arg);                                                                 // 372
                                                                                                        // 373
    if (arg.length > 2) {                                                                               // 374
      // keyword argument (represented as [type, value, name])                                          // 375
      kwArgs = (kwArgs || {});                                                                          // 376
      kwArgs[arg[2]] = argCode;                                                                         // 377
    } else {                                                                                            // 378
      // positional argument                                                                            // 379
      args = (args || []);                                                                              // 380
      args.push(argCode);                                                                               // 381
    }                                                                                                   // 382
  });                                                                                                   // 383
                                                                                                        // 384
  // put kwArgs in options dictionary at end of args                                                    // 385
  if (kwArgs) {                                                                                         // 386
    args = (args || []);                                                                                // 387
    args.push('Spacebars.kw(' + makeObjectLiteral(kwArgs) + ')');                                       // 388
  }                                                                                                     // 389
                                                                                                        // 390
  return args;                                                                                          // 391
};                                                                                                      // 392
                                                                                                        // 393
// Takes an inclusion tag and returns an object containing these properties,                            // 394
// all optional, whose values are JS source code:                                                       // 395
//                                                                                                      // 396
// - `dataFunc` - source code of a data function literal                                                // 397
// - `content` - source code of a content block                                                         // 398
// - `elseContent` - source code of an elseContent block                                                // 399
//                                                                                                      // 400
// Implements the calling convention for inclusions.                                                    // 401
var codeGenInclusionParts = function (tag) {                                                            // 402
  var ret = {};                                                                                         // 403
                                                                                                        // 404
  if ('content' in tag) {                                                                               // 405
    ret.content = (                                                                                     // 406
      'UI.block(' + Spacebars.codeGen(tag.content) + ')');                                              // 407
  }                                                                                                     // 408
  if ('elseContent' in tag) {                                                                           // 409
    ret.elseContent = (                                                                                 // 410
      'UI.block(' + Spacebars.codeGen(tag.elseContent) + ')');                                          // 411
  }                                                                                                     // 412
                                                                                                        // 413
  var dataFuncCode = null;                                                                              // 414
                                                                                                        // 415
  var args = tag.args;                                                                                  // 416
  if (! args.length) {                                                                                  // 417
    // e.g. `{{#foo}}`                                                                                  // 418
    return ret;                                                                                         // 419
  } else if (args[0].length === 3) {                                                                    // 420
    // keyword arguments only, e.g. `{{> point x=1 y=2}}`                                               // 421
    var dataProps = {};                                                                                 // 422
    _.each(args, function (arg) {                                                                       // 423
      var argKey = arg[2];                                                                              // 424
      dataProps[argKey] = 'Spacebars.call(' + codeGenArgValue(arg) + ')';                               // 425
    });                                                                                                 // 426
    dataFuncCode = makeObjectLiteral(dataProps);                                                        // 427
  } else if (args[0][0] !== 'PATH') {                                                                   // 428
    // literal first argument, e.g. `{{> foo "blah"}}`                                                  // 429
    //                                                                                                  // 430
    // tag validation has confirmed, in this case, that there is only                                   // 431
    // one argument (`args.length === 1`)                                                               // 432
    dataFuncCode = codeGenArgValue(args[0]);                                                            // 433
  } else if (args.length === 1) {                                                                       // 434
    // one argument, must be a PATH                                                                     // 435
    dataFuncCode = 'Spacebars.call(' + codeGenPath(args[0][1]) + ')';                                   // 436
  } else {                                                                                              // 437
    dataFuncCode = codeGenMustache(args[0][1], args.slice(1),                                           // 438
                                   'dataMustache');                                                     // 439
  }                                                                                                     // 440
                                                                                                        // 441
  ret.dataFunc = 'function () { return ' + dataFuncCode + '; }';                                        // 442
                                                                                                        // 443
  return ret;                                                                                           // 444
};                                                                                                      // 445
                                                                                                        // 446
...
// Main compiler                                                                                        // 449
                                                                                                        // 450
var replaceSpecials = function (node) {                                                                 // 451
  if (node instanceof HTML.Tag) {                                                                       // 452
    // potential optimization: don't always create a new tag                                            // 453
    var newChildren = _.map(node.children, replaceSpecials);                                            // 454
    var newTag = HTML.getTag(node.tagName).apply(null, newChildren);                                    // 455
    var oldAttrs = node.attrs;                                                                          // 456
    var newAttrs = null;                                                                                // 457
                                                                                                        // 458
    if (oldAttrs) {                                                                                     // 459
      _.each(oldAttrs, function (value, name) {                                                         // 460
        if (name.charAt(0) !== '$') {                                                                   // 461
          newAttrs = (newAttrs || {});                                                                  // 462
          newAttrs[name] = replaceSpecials(value);                                                      // 463
        }                                                                                               // 464
      });                                                                                               // 465
                                                                                                        // 466
      if (oldAttrs.$specials && oldAttrs.$specials.length) {                                            // 467
        newAttrs = (newAttrs || {});                                                                    // 468
        newAttrs.$dynamic = _.map(oldAttrs.$specials, function (special) {                              // 469
          return codeGenTemplateTag(special.value);                                                     // 470
        });                                                                                             // 471
      }                                                                                                 // 472
    }                                                                                                   // 473
                                                                                                        // 474
    newTag.attrs = newAttrs;                                                                            // 475
    return newTag;                                                                                      // 476
  } else if (node instanceof Array) {                                                                   // 477
    return _.map(node, replaceSpecials);                                                                // 478
  } else if (node instanceof HTMLTools.Special) {                                                       // 479
    return codeGenTemplateTag(node.value);                                                              // 480
  } else {                                                                                              // 481
    return node;                                                                                        // 482
  }                                                                                                     // 483
};                                                                                                      // 484
                                                                                                        // 485
Spacebars.compile = function (input, options) {                                                         // 486
  var tree = Spacebars.parse(input);                                                                    // 487
  return Spacebars.codeGen(tree, options);                                                              // 488
};                                                                                                      // 489
                                                                                                        // 490
Spacebars.codeGen = function (parseTree, options) {                                                     // 491
  // is this a template, rather than a block passed to                                                  // 492
  // a block helper, say                                                                                // 493
  var isTemplate = (options && options.isTemplate);                                                     // 494
                                                                                                        // 495
  var tree = parseTree;                                                                                 // 496
                                                                                                        // 497
  // The flags `isTemplate` and `isBody` are kind of a hack.                                            // 498
  if (isTemplate || (options && options.isBody)) {                                                      // 499
    // optimizing fragments would require being smarter about whether we are                            // 500
    // in a TEXTAREA, say.                                                                              // 501
    tree = optimize(tree);                                                                              // 502
  }                                                                                                     // 503
                                                                                                        // 504
  tree = replaceSpecials(tree);                                                                         // 505
                                                                                                        // 506
  var code = '(function () { var self = this; ';                                                        // 507
  if (isTemplate) {                                                                                     // 508
    // support `{{> UI.contentBlock}}` and `{{> UI.elseBlock}}` with                                    // 509
    // lexical scope by creating a local variable in the                                                // 510
    // template's render function.                                                                      // 511
    code += 'var template = this; ';                                                                    // 512
  }                                                                                                     // 513
  code += 'return ';                                                                                    // 514
  code += HTML.toJS(tree);                                                                              // 515
  code += '; })';                                                                                       // 516
                                                                                                        // 517
  code = beautify(code);                                                                                // 518
                                                                                                        // 519
  return code;                                                                                          // 520
};                                                                                                      // 521
                                                                                                        // 522
var beautify = function (code) {                                                                        // 523
  if (Package.minifiers && Package.minifiers.UglifyJSMinify) {                                          // 524
    var result = UglifyJSMinify(code,                                                                   // 525
                                { fromString: true,                                                     // 526
                                  mangle: false,                                                        // 527
                                  compress: false,                                                      // 528
                                  output: { beautify: true,                                             // 529
                                            indent_level: 2,                                            // 530
                                            width: 80 } });                                             // 531
    var output = result.code;                                                                           // 532
    // Uglify interprets our expression as a statement and may add a semicolon.                         // 533
    // Strip trailing semicolon.                                                                        // 534
    output = output.replace(/;$/, '');                                                                  // 535
    return output;                                                                                      // 536
  } else {                                                                                              // 537
    // don't actually beautify; no UglifyJS                                                             // 538
    return code;                                                                                        // 539
  }                                                                                                     // 540
};                                                                                                      // 541
                                                                                                        // 542
// expose for compiler output tests                                                                     // 543
Spacebars._beautify = beautify;                                                                         // 544
                                                                                                        // 545
//////////////////////////////////////////////////////////////////////////////////////////////////////////
