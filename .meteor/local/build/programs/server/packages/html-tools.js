...
getHTMLToken = HTMLTools.Parse.getHTMLToken = function (scanner, dataMode) {                                   // 208
  ...
  if (scanner.getSpecialTag) {                                                                                 // 210
    // Try to parse a "special tag" by calling out to the provided                                             // 211
    // `getSpecialTag` function.  If the function returns `null` but                                           // 212
    // consumes characters, it must have parsed a comment, so we return null                                   // 213
    // and allow the lexer to continue.  If it ever returns `null` without                                     // 214
    // consuming anything, that means it didn't see anything interesting                                       // 215
    // so we look for a normal token.  If it returns a truthy value,                                           // 216
    // the value must be an object.  We wrap it in a Special token.                                            // 217
    var lastPos = scanner.pos;                                                                                 // 218
    result = scanner.getSpecialTag(                                                                            // 219
      scanner,                                                                                                 // 220
      (dataMode === 'rcdata' ? TEMPLATE_TAG_POSITION.IN_RCDATA :                                               // 221
       (dataMode === 'rawtext' ? TEMPLATE_TAG_POSITION.IN_RAWTEXT :                                            // 222
        TEMPLATE_TAG_POSITION.ELEMENT)));                                                                      // 223
                                                                                                               // 224
    if (result)                                                                                                // 225
      return { t: 'Special', v: result };                                                                      // 226
    else if (scanner.pos > lastPos)                                                                            // 227
      return null;                                                                                             // 228
  }                                                                                                            // 229
                                                                                                               // 230
